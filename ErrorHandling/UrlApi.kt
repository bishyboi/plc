...
class HttpError(val status: Int, message: String, cause: Throwable? = null) :
    Exception("${message} (status=${status})", cause) {
...
object UrlApi {
    ...
    fun create(body: Map<String, String>): HttpSuccess<UrlModel> {
        ...
        if (body.size > 2 || (body.size == 2 && !body.containsKey("alias"))) {
            throw HttpError(400, "Invalid body")
        }

        return try {
            HttpSuccess(201, UrlModel.insert(url, body["alias"]))
        } catch (e: IOException) {
            throw HttpError(500, e)
        } catch (e: IllegalArgumentException) {
            // Translate alias conflict to 489; let other contract errors bubble up
            if (e.message?.contains("alias") == true) throw HttpError(489, e) else throw e
        }
    }
    ...
    fun read(key: String): HttpSuccess<UrlModel> {
        ...
        val model = try {
            if (id != null) UrlModel.select(id)
            else UrlModel.select(key) ?: throw NoSuchElementException()
    ...
    fun update(id: String, body: Map<String, String>): Result<HttpSuccess<UrlModel>> {
        val intId = id.toIntOrNull() ?: return Result.failure(HttpError(400, "Invalid ID"))
        val alias = body["alias"] ?: return Result.failure(HttpError(400, "Missing alias"))

        if (body.size > 1) return Result.failure(HttpError(400, "Invalid fields"))

        return UrlModel.update(intId, alias)
            .fold(onSuccess = { Result.success(HttpSuccess(200, it)) }, onFailure = { ex ->
                when (ex) {
                    is NoSuchElementException -> Result.failure(HttpError(404, ex))
                    is IllegalArgumentException -> throw ex // Contract error
                    else -> Result.failure(HttpError(500, ex))
                }
            })
    }
    ...
    fun delete(id: String): Boolean {
        ...
        return UrlModel.delete(intId).getOrElse { ex ->
            // Re-throw server/contract errors; return false for standard failures
            throw ex
        }
    }
...
data class UrlModel(val id: Int, val url: String, val alias: String?) {
    ...
    companion object {
        ...
        fun resetDB() {
            DB.id = 1; DB.records.clear(); DB.aliases.clear()
        }
        ...
        fun update(id: Int, alias: String?): Result<UrlModel> {
            ...
            val current =
                DB.records[id] ?: return Result.failure(NoSuchElementException("Record ${id} does not exist."))
...
fun main() {
    ...
    test(
        "Read DB Error", listOf(
            "GET /urls/error" to HttpError(500, "unused"),
        )
    )
    test(
        "Read Contract Error", listOf(
    ...
    test(
        "Delete Missing", listOf(
            "DELETE /urls/1" to false,
        )
    )
    test(
        "Delete Invalid", listOf(
            "DELETE /urls/invalid" to false,
        )
    )
    test(
        "Delete DB Error", listOf(
            "DELETE /urls/-1" to IOException::class.java,
        )
    )
    test(
        "Delete Contract Error", listOf(
...
private fun parseBody(string: String): Map<String, String> {
    ...
    return string.removeSurrounding("{", "}").split(",").takeUnless { it.size == 1 && it[0] == "" }.orEmpty()
        .associate {
